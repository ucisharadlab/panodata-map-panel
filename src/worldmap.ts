...
export default class WorldMap {
  ...
  drawShapes() {
    const data = this.filterEmptyAndZeroValues(this.ctrl.data);
    if (this.needToRedrawShapes(data)) {
      console.info('Creating shapes');
      this.clearShapes();
  ...
  createShapes(data) {
    ...
    data.forEach((dataPoint) => {
      let shape;
      shape = this.createShape(dataPoint);
      shapes.push(shape);
    });
    this.shapesLayer = this.addShapes(shapes);
    this.shapes = shapes;
  }

  needToRedrawShapes(data) {
    console.info(`Data points ${data.length}. Circles on map ${this.circles.length}.`);
  ...
  removeShapes() {
    this.map.removeLayer(this.shapesLayer);
  }

  addShapes(shapes) {
    // Todo: Optionally add fixed custom attributions to the shape layer.
    const attribution = undefined;
    return (window as any).L.layerGroup(shapes, { attribution: attribution }).addTo(this.map);
  }

  createShape(dataPoint) {
    var vertices = JSON.parse(dataPoint.LocationVertices.replaceAll('{', '[').replaceAll('}', ']'));
    var radius = JSON.parse(dataPoint.LocationRadius);
    if (dataPoint.LocationShape === 'rectangle') {
      return (window as any).L.rectangle(vertices).addTo(this.map);
    } else if (dataPoint.LocationShape === 'circle') {
      return (window as any).L.circle(vertices[0], { radius: radius }).addTo(this.map);
    } else if (dataPoint.LocationShape === 'polygon') {
      return (window as any).L.polygon(vertices).addTo(this.map);
    }
  ...
  panToMapCenter(options?: any) {
    ...
    if (mapDimensions.mapFitData) {
      ...
      if (this.shapesLayer) {
        const group = L.featureGroup(this.shapesLayer.getLayers());
        const bounds = group.getBounds();
        if (!_.isEmpty(bounds)) {
          coordinates = bounds.getCenter();
          zoomLevel = this.map.getBoundsZoom(bounds);
